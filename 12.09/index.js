...
function filterRecipes(guest, recipes) {
  ...
  for (let recipe of recipes) {
    if (recipe.ingredients.filter(ingredient => guest.dislikes.includes(ingredient)).length === 0 &&
        recipe.ingredients.filter(ingredient => guest.loves.includes(ingredient)).length > 0) {
          result.push(recipe);
    }
  }
...
console.log(filterRecipes(guest, recipes));
